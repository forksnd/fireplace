 ...
 */
public class LineInlayRenderer extends InlayRenderer {
     ...
     */
    public LineInlayRenderer(Stroke stroke, Paint paint) {
     ...
     */
    @Override
    public void draw(InlayChart chart, XYDataset dataset, Graphics2D g2, Rectangle2D plotBounds) {
        ...
        var xRange = dataset.getRangeOfX();
        var yRange = dataset.getRangeOfY();
        ...
        for (int i = 0; i < itemCount; i++) {
            ...
            double xx = plotBounds.getX() + xRange.calcFraction(dataset.getX(i)) * plotBounds.getWidth();
            if (dataset.getY(i) == null) continue;
            double yy = plotBounds.getMaxY() - yRange.calcFraction(dataset.getY(i)) * plotBounds.getHeight();
            if (i == 0) {
                path.moveTo(xx, yy);
            } else {
                path.lineTo(xx, yy);
